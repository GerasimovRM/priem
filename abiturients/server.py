...
exclude_data = {"ist": 65, "fop": 25, "pm": 40, "ivk": 25, "mie": 25, "mm": 10}
...
@app.get("/get_default")
async def get_default(group_name: str):
    if group_name not in group_names:
        raise HTTPException(status_code=404, detail="Group not found")
    client = DbClient()
    abiturients = client["abiturients"]
    return {group_name: update_rating(abiturients[group_name].find({}))}


@app.get("/get_original_docs_without_agreement")
async def get_original_docs_without_agreement(group_name: str):
    if group_name not in group_names:
        raise HTTPException(status_code=404, detail="Group not found")
    client = DbClient()
    query = {"Вид документа об образовании": "Оригинал", "Согласие на зачисление": None}
    abiturients = client["abiturients"]
    return {group_name: update_rating(abiturients[group_name].find(query))}


@app.get("/get_agreement_without_original_docs")
async def get_agreement_without_original_docs(group_name: str):
    if group_name not in group_names:
        raise HTTPException(status_code=404, detail="Group not found")
    client = DbClient()
...
async def get_agreement_with_original_docs(group_name: str):
    if group_name not in group_names:
        raise HTTPException(status_code=404, detail="Group not found")
    client = DbClient()
    abiturients = client["abiturients"]
...
async def get_exclude_peoples(group_name: str):
    if group_name not in group_names:
        raise HTTPException(status_code=404, detail="Group not found")
    client = DbClient()
    abiturients = client["abiturients"]
    ...
    return {group_name: update_rating(abiturients[group_name].find(query)[exclude_data[group_name]:],
                                      enumerate_with=exclude_data[group_name] + 1)}
