...
def zip_to_dir(zip_path):
    ...
    direction_name = zip_path.split("\\")[-2].split(".")[0]
    dst = f"{DATA_DIR}\\{direction_name}\\{zip_name}"
...
subfolders = [f.path for f in os.scandir(ZIP_DIR) if f.is_dir()]
for zip_dir in subfolders:
    direction_name = zip_dir.split("\\")[-1]
    ...
    for student_name in next(os.walk(DATA_DIR))[1]:
        for root, dirs, files in list(os.walk(f"{DATA_DIR}\\{direction_name}\\{student_name}"))[::-1]:
